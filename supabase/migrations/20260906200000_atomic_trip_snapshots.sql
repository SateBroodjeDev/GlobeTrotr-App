-- GlobeTrotr: sla een volledige reis atomair op.
--
-- Voer uit na de eerdere GlobeTrotr-migraties. Deze functie is uitsluitend
-- bedoeld voor de serverfunctie met de service-role; de browser krijgt geen
-- directe uitvoerrechten. Als een kindrij ongeldig is, draait PostgreSQL de
-- gehele reiswijziging terug, inclusief de tijdelijke JSON-compatibiliteitskopie.

BEGIN;

-- De snapshot-writer gebruikt de rijkere boekingsvelden. Deze statements zijn
-- bewust idempotent, zodat de migratie ook veilig is wanneer 18000 al is
-- uitgevoerd.
...
BEGIN
  ...
  DELETE FROM public.trip_stops WHERE trip_uuid = v_trip_uuid;
  ...
  DELETE FROM public.trip_itinerary_items WHERE trip_uuid = v_trip_uuid;
  ...
  DELETE FROM public.trip_expenses WHERE trip_uuid = v_trip_uuid;
  ...
  DELETE FROM public.trip_travel_items WHERE trip_uuid = v_trip_uuid;
  ...
  DELETE FROM public.trip_packing_items WHERE trip_uuid = v_trip_uuid;
