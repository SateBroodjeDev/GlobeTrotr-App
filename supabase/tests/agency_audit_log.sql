...
DO $$ BEGIN
  IF (SELECT count(*) FROM public.agency_audit_log
      WHERE workspace_uuid=(SELECT workspace_id FROM agency_audit_ids)
        AND target_id='client-test')<>1 THEN RAISE EXCEPTION 'Agency-auditregel ontbreekt'; END IF;
