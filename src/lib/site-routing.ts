...
const portalPaths =
  /^\/(?:auth|register|oauth-callback|complete-profile|session-bridge|token|dashboard|trips|account|billing|agency-admin|corporate-admin|company-mail|branding|team|analytics|client-portal|invite|uitnodiging|agency-invite|agency-uitnodiging)(?:\/|$)/;
const websitePaths =
  /^\/(?:demo|features|mogelijkheden|prijzen|pricing|about|for-groups|for-travelers|for-agencies|roadmap|updates|changelog|privacy|algemene-voorwaarden|terms|beta-voorwaarden|terugbetalingsbeleid|refund-policy|beta|known-issues|bekende-problemen|trip|reis|quote)(?:\/|$)/;
...
export function canonicalSiteLocation(hostname: string, pathname: string, search = "", hash = "") {
  ...
  if (
    !platformHost &&
    (pathname === "/" ||
      websitePaths.test(pathname) ||
      /^\/(?:contact|status)(?:\/|$)/.test(pathname))
  )
