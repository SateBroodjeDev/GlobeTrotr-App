  ...
  .handler(async ({ data, context }) => {
    ...
    const selectedId =
      data.mailboxId && unique.some((m) => m.id === data.mailboxId)
        ? data.mailboxId
        : unique[0]?.id;
    if (!selectedId) return { mailboxes: [], messages: [], outbox: [], selected: null };
    const selected = unique.find((m) => m.id === selectedId);
    const [messages, outbox] = await Promise.all([
      db
        .from("corporate_mail_messages")
        .select(
          "id,direction,sender_address,recipient_addresses,subject,preview_text,body_text,received_at,read_at,archived_at",
        )
      ...
      db
        ...
        .select(
          "id,recipient_addresses,cc_addresses,subject,status,attempts,sent_at,last_error_code,created_at",
        )
        .eq("mailbox_id", selectedId)
        .order("created_at", { ascending: false })
        .limit(100),
    ]);
    if (messages.error || outbox.error) throw new Error("MAIL_LOAD_FAILED");
    return {
      mailboxes: unique,
...
export const queueCorporateMail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const body = `${data.body.trim()}${access.mailbox.signature_text ? `\n\n${access.mailbox.signature_text}` : ""}`;
    const { data: config } = await db
      .from("email_delivery_config")
      .select("mode")
      .eq("id", true)
...
export const updateCorporateMailMessage = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator(
    (input: { mailboxId: string; messageId: string; action: "read" | "unread" | "archive" }) =>
      input,
  )
  .handler(async ({ data, context }) => {
    const db = await staffDb(context.userId);
...
export const saveCorporateStaff = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!user.data.user) throw new Error("USER_NOT_FOUND");
