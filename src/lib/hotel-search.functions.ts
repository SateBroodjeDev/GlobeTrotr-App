...
export const searchNearbyHotels = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const key = `${data.lat.toFixed(3)}:${data.lon.toFixed(3)}`;
    ...
    const query = `[out:json][timeout:10];nwr["tourism"~"^(hotel|hostel|guest_house|motel|apartment|chalet)$"](around:5000,${data.lat},${data.lon});out center tags 30;`;
    const response = await fetch(endpoint, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": "GlobeTrotr/1.1 (https://globetrotr.nl/contact)" }, body: new URLSearchParams({ data: query }), signal: AbortSignal.timeout(12_000) });
    if (!response.ok) throw new Error("HOTEL_SEARCH_UNAVAILABLE");
