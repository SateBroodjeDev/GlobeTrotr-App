...
test("reiskopie bewaart planning maar verwijdert toegang en gevoelige boekingsdata", () => {
  ...
  const source = {
    ...
    budget: 500,
    stops: [],
    itinerary: [{ id: "day", day: "2026-01-01", title: "Rijden", sourceTravelItemId: "booking" }],
    travelItems: [
      {
        id: "booking",
        type: "lodging",
  ...
  const copy = buildTripDuplicate(
    source,
    { ...source, id: "new", revision: "1", expenses: [] },
    () => `new-${++sequence}`,
  );
  ...
  assert.deepEqual(copy.travelOptions, []);
