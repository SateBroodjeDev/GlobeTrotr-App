...
test("marketing paths leave the portal while contact and status stay available", () => {
  assert.equal(canonicalSiteLocation("portal.globetrotr.nl", "/"), "https://portal.globetrotr.nl/dashboard");
  assert.equal(canonicalSiteLocation("portal.globetrotr.nl", "/demo"), "https://globetrotr.nl/demo");
  ...
  assert.equal(canonicalSiteLocation("agency.globetrotr.nl", "/"), "https://agency.globetrotr.nl/agency-admin");
  assert.equal(canonicalSiteLocation("test.example.nl", "/"), "https://test.example.nl/agency-admin");
