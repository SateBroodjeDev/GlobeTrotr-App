...
export function TripReadiness({ trip, text }: { trip: Trip; text: (nl: string, en: string) => string }) {
  ...
  const rows = [
    ...
    { icon: ListChecks, ready: readiness.tripDays > 0 && readiness.plannedDays >= readiness.tripDays, label: text("Dagplanning", "Daily plan"), value: readiness.tripDays ? `${readiness.plannedDays}/${readiness.tripDays} ${text("reisdagen ingevuld", "trip days planned")}` : text("Controleer de reisdatums", "Check the trip dates") },
