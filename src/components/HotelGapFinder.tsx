...
import { BedDouble, ExternalLink, Loader2, MapPin, Search } from "lucide-react";
  ...
  return <>
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><BedDouble className="size-5" />{text("Hotelcontrole en zoeken", "Accommodation check and search")}</CardTitle></CardHeader><CardContent className="space-y-5">
      ...
      {gaps.map((item) => {
        ...
        return <div key={item.id} className="grid gap-3 rounded-lg border p-3 md:grid-cols-[minmax(10rem,1fr)_minmax(12rem,1fr)_auto] md:items-end"><div><strong>{item.startDate} – {item.endDate}</strong><p className="text-sm text-muted-foreground">{item.nights} {item.nights === 1 ? text("nacht zonder verblijf", "night without accommodation") : text("nachten zonder verblijf", "nights without accommodation")}</p></div><label className="space-y-1 text-sm"><span className="font-medium">{suggestion ? text("Waarschijnlijk overnachten in", "Likely stay in") : text("Waar wil je overnachten?", "Where do you want to stay?")}</span><select className="h-10 w-full rounded-md border bg-background px-3" value={selected} disabled={!editable} onChange={(event) => setLocations((current) => ({ ...current, [item.id]: event.target.value }))}><option value="">{text("Kies een routeplaats", "Choose a route location")}</option>{trip.stops.map((stop) => <option key={stop.id} value={stop.id}>{stop.name}{stop.country ? ` · ${stop.country}` : ""}</option>)}</select>{item.confidence === "route" && <span className="block text-xs text-muted-foreground">{text("Ingeschat via het laatst bereikte routepunt; controleer dit.", "Estimated from the latest reached route point; please verify.")}</span>}</label><Button disabled={!editable || loading || !selected} onClick={() => void search(item)}><Search className="size-4" />{text("Zoek verblijven", "Find stays")}</Button></div>;
      })}
    ...
    <Dialog open={Boolean(gap)} onOpenChange={(open) => { if (!open) setGap(undefined); }}><DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{text("Verblijven rond", "Accommodation near")} {gap?.stopName}</DialogTitle></DialogHeader>{gap && <p className="text-sm text-muted-foreground">{gap.startDate} – {gap.endDate} · {text("gesorteerd op afstand", "ordered by distance")}</p>}{loading ? <div className="flex items-center justify-center gap-2 p-10"><Loader2 className="size-5 animate-spin" />{text("Verblijven zoeken…", "Searching accommodation…")}</div> : !results.length ? <p className="py-8 text-center text-sm text-muted-foreground">{text("Geen benoemde verblijven binnen 5 km gevonden.", "No named accommodation found within 5 km.")}</p> : <div className="space-y-2">{results.map((result) => <div key={result.id} className="rounded-lg border p-3"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><strong>{result.name}</strong><p className="flex items-center gap-1 text-sm text-muted-foreground"><MapPin className="size-3.5" />{result.kind.replaceAll("_", " ")} · {result.distanceKm} km{result.stars ? ` · ${result.stars}★` : ""}</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" size="sm" asChild><a href={result.website ?? result.osmUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="size-4" />{text("Bekijken", "View")}</a></Button><Button size="sm" disabled={!editable || Boolean(saving)} onClick={() => void add(result)}>{saving === result.id && <Loader2 className="size-4 animate-spin" />}{text("Bewaar als kandidaat", "Save as candidate")}</Button></div></div></div>)}</div>}<p className="text-xs text-muted-foreground">© <a className="underline" href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">OpenStreetMap contributors</a></p></DialogContent></Dialog>
  </>;
}
