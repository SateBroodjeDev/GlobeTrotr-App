...
export function HotelGapFinder({ trip, editable, save, text }: Props) {
  ...
  async function search(item: TripHotelGap) {
    ...
    const inferred = item.suggestedLocation;
    const location = stop ?? inferred;
    if (!location) {
      toast.error(text("Kies eerst waar je deze nacht wilt overnachten.", "First choose where you want to stay that night."));
      return;
    }
    const resolved: HotelGap = { ...item, stopId: stop?.id ?? `booking:${item.id}`, stopName: location.name, country: location.country, lat: location.lat, lon: location.lon };
