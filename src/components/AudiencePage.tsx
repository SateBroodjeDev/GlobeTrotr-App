...
export function AudiencePage({ badge, title, intro, highlights, workflow }: {
  ...
  return <div className="space-y-14 pb-8">
    <section className="aurora overflow-hidden rounded-[2rem] px-6 py-12 sm:px-10 lg:px-14">
      ...
      <div className="mt-8 flex flex-wrap gap-3"><Button asChild size="lg"><Link to={user ? "/dashboard" : "/auth"}>{user ? text("Open mijn reizen", "Open my trips") : text("Gratis beginnen", "Start for free")}<ArrowRight className="size-4" /></Link></Button><Button asChild size="lg" variant="outline"><Link to="/demo">{text("Bekijk de demo", "View the demo")}</Link></Button></div>
    </section>
