...
export function TripOfflinePack({ trip, text, canManageExpenses, payers, syncExpenses }: Props) {
  ...
  const queued = pack?.queuedExpenses ?? [];
  ...
  async function sync() {
    if (!queued.length || !online) return;
    setBusy(true);
    try {
      ...
      setPack((current) => current ? { ...current, queuedExpenses: [] } : current);
  ...
  return <Card className="surface">
    ...
    <CardContent className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {queued.length > 0 && canManageExpenses && <Button type="button" size="sm" variant="secondary" onClick={() => void sync()} disabled={busy || !online}><RefreshCw className="size-4"/>{text("Uitgaven synchroniseren", "Sync expenses")}</Button>}
      </div>
    </CardContent>
  </Card>;
}
