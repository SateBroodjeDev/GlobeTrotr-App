...
export function TripTasks({ tripId, editable, text }: { tripId: string; editable: boolean; text: (nl: string, en: string) => string }) {
  ...
  const open = useMemo(() => tasks.filter((task) => !task.completed).length, [tasks]);
  const save = async () => {
    if (title.trim().length < 2) return;
    setSaving(true);
    try {
      await saveTripTask({ data: { tripId, title, assigneeName, dueDate: dueDate || null, completed: false } });
  ...
  const update = async (task: TripTask, completed: boolean) => {
    try {
      await saveTripTask({ data: { tripId, id: task.id, title: task.title, assigneeName: task.assigneeName, dueDate: task.dueDate, completed } });
  ...
  return <Card className="surface">
    <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><ListTodo className="size-4"/>{text("Reistaken", "Trip tasks")}<span className="ml-auto text-xs font-normal text-muted-foreground">{open} {text("open", "open")}</span></CardTitle></CardHeader>
    <CardContent className="space-y-4">
      {editable && <div className="grid gap-3 rounded-xl border bg-muted/20 p-3 md:grid-cols-[1fr_180px_150px_auto] md:items-end">
        ...
        <Button disabled={saving || title.trim().length < 2} onClick={save}><Plus className="size-4"/>{text("Toevoegen", "Add")}</Button>
      </div>}
      {loading ? <p className="text-sm text-muted-foreground">{text("Taken laden…", "Loading tasks…")}</p> : tasks.length === 0 ? <p className="text-sm text-muted-foreground">{text("Nog geen taken voor deze reis.", "No tasks for this trip yet.")}</p> : <div className="divide-y rounded-xl border">{tasks.map((task)=><div key={task.id} className="flex items-center gap-3 p-3">
