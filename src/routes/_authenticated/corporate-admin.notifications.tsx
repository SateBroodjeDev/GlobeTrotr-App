...
import { CheckCircle2, Languages, Megaphone, Pencil, Send } from "lucide-react";
...
function PlatformNotifications() {
  ...
  async function translateToEnglish() {
    if (!form.titleNl.trim() || !form.bodyNl.trim()) return;
    setBusy(true);
    try {
      const [title,body]=await Promise.all([
        createTranslationDraft({data:{text:form.titleNl,source:"nl",target:"en"}}),
        createTranslationDraft({data:{text:form.bodyNl,source:"nl",target:"en"}}),
      ]);
      setForm(current=>({...current,titleEn:title.translated,bodyEn:body.translated}));
      toast.success(text("Engels concept gemaakt. Controleer het voor publicatie.","English draft created. Review it before publishing."));
    } catch(error) {
