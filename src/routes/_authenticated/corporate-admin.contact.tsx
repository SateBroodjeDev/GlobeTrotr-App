...
function ContactInbox() {
  ...
  return <div className="space-y-4">
    ...
    {query.isLoading ? <Card><CardContent className="p-6 text-sm text-muted-foreground">{text("Berichten laden…", "Loading messages…")}</CardContent></Card> : query.isError ? <Card><CardContent className="p-6 text-sm text-destructive">{text("De contactberichten konden niet worden geladen.", "Contact messages could not be loaded.")}</CardContent></Card> : !rows.length ? <Card><CardContent className="flex items-center gap-3 p-6 text-sm text-muted-foreground"><Inbox className="size-5"/>{text("Geen berichten binnen dit filter.", "No messages in this filter.")}</CardContent></Card> : <div className="grid gap-4 xl:grid-cols-[minmax(18rem,0.8fr)_minmax(0,1.4fr)]">
      ...
      {active && <Card><CardHeader><div className="flex flex-wrap items-start gap-3"><div className="min-w-0 flex-1"><CardTitle>{active.subject}</CardTitle><p className="mt-2 text-sm text-muted-foreground">{active.name} · {active.email}</p></div><Button asChild variant="outline" size="sm"><Link to="/company-mail" search={{to:active.email,subject:`Re: ${active.subject}`}}><Mail className="size-4"/>{text("Beantwoorden via bedrijfsmail", "Reply using company mail")}<ExternalLink className="size-3"/></Link></Button></div></CardHeader><CardContent className="space-y-5"><p className="whitespace-pre-wrap rounded-xl border bg-muted/30 p-4 text-sm leading-relaxed">{active.message}</p><div className="flex flex-wrap items-center gap-3"><label className="text-sm font-medium" htmlFor="contact-status">{text("Status", "Status")}</label><select id="contact-status" disabled={busy} className="h-10 rounded-md border bg-background px-3" value={active.status} onChange={e => void change(active.id, e.target.value as Status)}>{statuses.map(status => <option key={status} value={status}>{text(...labels[status])}</option>)}</select><span className="text-xs text-muted-foreground">{text("Het antwoord wordt vanuit een gekoppelde GlobeTrotr-mailbox verzonden.", "The reply is sent from a linked GlobeTrotr mailbox.")}</span></div></CardContent></Card>}
    </div>}
  </div>;
}
