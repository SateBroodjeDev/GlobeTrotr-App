...
function KnownIssuesPage() {
  ...
  const query = useQuery({ queryKey:["known-issues"], queryFn: async()=>{ const {data,error}=await (supabase as any).from("known_issues").select("*").eq("public",true).order("created_at",{ascending:false}); if(error) throw error; return data as any[]; }});
